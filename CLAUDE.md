...
- **Never test CLV against a ratio of 1.0 or a 50% shortening rate. Measure the null.** The pre-close and the close are not on average the same price: Pinnacle's overround tightens toward kickoff in every season measured, so prices lengthen by default and a randomly chosen selection inside the rule's odds band shortens only **45–48%** of the time — 31% in the 2025-26 *lower tiers*, 38.9% on Phase 6's all-division population that season. Against 50% a real effect reads as nothing and a null tier reads as a contradiction; both happened here in one session and both were recorded before being caught. Match the null's odds distribution to the model's own bets (`scripts/h1_odds_matched_null.py`) — though matched and unmatched agreed to within 0.002 in the H1 window and 0.004 on Phase 6's, so the mix is not usually the driver. **Both original pre-registrations here were written against the wrong null**, and correcting it flipped the sign of the founding study's CLV conclusion.
...
The harness self-tests are the reason any number here is trustworthy. `uv run pytest` — **337 tests** (2026-08-17).
