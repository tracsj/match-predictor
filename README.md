...
**Its betting numbers should not be trusted.** Five of its scripts assume the first-listed team is the home team; measured across 640 fixtures, that holds 63.6% of the time, so roughly a third of simulated bets were graded against the wrong side's price. `v1/scripts/build_web_bundle.py` is the exception. It resolves home and away correctly, which makes its bundle and the browser dashboard the only reliable betting output in v1, and v2 gets a regression test for exactly this. The bundle is not in this repo: it carries SportMonks fixture ids and bookmaker odds, which are theirs to redistribute rather than mine. Run that script against your own `data/` and the dashboard renders.
