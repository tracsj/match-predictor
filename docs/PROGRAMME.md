...
**H1 — settled, supported, and not the finding it claimed.** Its pre-registered test passed (lower stratum CLV 1.0083, 52.53% shortened, 9,920 bets, p < 0.0001), recorded *before* any control ran. The controls then corrected three of its readings. Against each tier's own drift, **tier 1 is the only tier indistinguishable from its own baseline** (+0.70pp, z = 1.35); tiers 2–5 run +4.16, +7.68, +8.65, +4.25pp. So the model anticipates line movement everywhere except the top flight, most strongly in tiers 3–4, and the gradient is not monotone. Out of sample on 2025-26: lower +6.74pp (z 2.55) against +7.09pp in sample — directionally consistent, **not** a replication, and inconclusive by the 3,250-bet floor. `docs/H1_RESULT.md`.

**Phase 6 — its CLV reading is withdrawn.** Pre-registered at `docs/PREREG_PHASE6_NULL.md`, gated on reproducing the published row (it did, to four decimals). Observed 0.4241 shortened against an odds-matched null of 0.3926 — **above the interval, +3.15pp at p = 0.018**. Narrow, and short of the p < 0.01 bar. Superseded passages are marked in place rather than rewritten. **Its ROI tables and "the rule lost money" conclusion are untouched.**

**H3 — supported, and useless, which is the most useful thing here.** Movement direction is forecastable: +4.27pp over a matched null, z 6.96, p 3e-12, accuracy 44.39% against a 39.89% baseline. But the gain is **−0.15% of price against a 5.08% margin**, so it is a forecasting result and not a strategy. And on 6,173 shared rows it did **not** beat the match model's incidental signal (+3.98pp vs +4.50pp, p 0.65). **The market moves toward what a decent match model already thinks, with no separable microstructure signal on top.** `docs/H3_RESULT.md`.

**What that means for the programme's direction.** H3 was the registry's strongest remaining candidate and it is now settled negative in the way that matters. The 1X2 football market has been examined from the match side, the tier side and the market side, and all three land in the same place: real but sub-margin signal. **New work should go to a different market, not a different angle on this one** — which is what `CLAUDE.md` has said since the founding study, now with three converging examinations behind it.
