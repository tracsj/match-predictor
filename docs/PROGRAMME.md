...
2. ~~**H1 pre-registered**~~ — **done 2026-08-17**, committed before any tier-stratified number existed. The three open items are closed: one absolute primary test (lower-stratum CLV > 1.0 at p < 0.01), a 3,250-bet floor per stratum derived from a power calculation rather than chosen, and untiered divisions (B1, G1, N1, P1, T1) excluded outright. **Next: build `src/h1.py` and run it.** It reuses `run_walk_forward` unmodified, so the model configuration is provably the settled one. Graded window is 2015-16 → 2024-25 because `min_train_seasons=3` is hardcoded — accepted, not worked around, and the reason is in the file.
